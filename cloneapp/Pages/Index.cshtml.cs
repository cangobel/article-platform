...
public class IndexModel : PageModel
{
	...
	public IActionResult OnGetCategoryArticlesProps(string category, int lastArticleId)
	{
		...
		return array.Count() != 0 ? new JsonResult(new { contents = array, lastArticleId = array.Last().ArticleId }) : new JsonResult(null);
	}

	public IActionResult OnGetExploreArticlesProps(int lastArticleId)
	{
		var array = GetArticlePropsArray(lastArticleId);

		return array.Count() != 0 ? new JsonResult(new { contents = array, lastArticleId = array.Last().ArticleId }) : new JsonResult(null);
	}

	public IEnumerable<ArticleProps> GetArticlePropsArray(int lastArticleId = 0)
	{
		return GetArticlePropsQuery(lastArticleId).Take(MaxQueryOutput).ToArray();
	}
